...
1. **Analyse der Hardware-Dimensionen:** Verifizierung der exakten Maße für das 16x16 Panel (160x160mm) und das 8x32 Panel (80x320mm).
2. **Recherche Ring-Parameter:** Bestimmung der Standardabmessungen für den 20-LED Ring als Standardvorgabe.
3. **OpenSCAD Umgebung einrichten:** Erstellung der Basis-Datei mit globalen Variablen.
4. **Definition des LED-Pitch:** Implementierung einer Variablen für den Mittenabstand der LEDs (z.B. 10mm).
5. **Parameter für Wandstärke:** Festlegung der minimalen Druckbreite für die Trennwände (z.B. 0.8mm oder 1.2mm).
6. **Definition der Diffusionshöhe:** Variable für den Abstand zwischen LED und Diffusor-Oberfläche zur Lichtmischung.
7. **Modellierung einer Einzelzelle:** Erstellung eines Moduls für eine quadratische Kammer.
...
21. **Python-Wrapper erstellen:** Entwicklung eines Skripts zur Steuerung von OpenSCAD via Kommandozeile.
22. **Automatisierter STL-Export:** Skriptgesteuerter Export für alle Zielgrößen (16x16, 8x32, Ring).
...
27. **Batch-Processing:** Möglichkeit, alle Varianten mit einem einzigen Befehl zu generieren.
